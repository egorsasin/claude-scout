---
name: migrate-plan
description: "Generates a phased migration plan from scout-context/ and migration/compat.md. Reads current documentation (components, routes, state, API layer) and the confirmed dependency mapping, then produces a sequenced plan with phases, tasks, effort estimates, and risks. Writes migration/plan.md. Part of the /migrate workflow — called by /migrate plan."
...
Reads `scout-context/` and `migration/compat.md`, then produces a phased,
sequenced migration plan saved to `migration/plan.md`.

## Input

- `migration/target.md` — confirmed target stack
- `migration/compat.md` — confirmed dependency mapping
...
### Step 5 — Write migration/plan.md
...
> Source: scout-context/ + migration/compat.md
...
```
✓ Migration plan written to migration/plan.md
...
`Spec: migration/components/widgets/{WidgetName}.md` so the developer knows
